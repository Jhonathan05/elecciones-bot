...
function conversar(ctx, phone, instance, mensajes) {
  let last = '';
  for (const m of mensajes) last = flows.handle(phone, m, ctx, instance);
  return last;
}
...
// ---- 1) Acta 021 con cuadre correcto ----
{
  ...
  const r = conversar(ctx, '573001112233', 'chaparral', [
...
// ---- 6) Línea no asociada rechaza ----
{
  ...
  const r = flows.handle('573009999999', 'hola', ctx, 'desconocida');
...
// ---- 7) Validación por asignación de coordinador de mesa ----
{
  ...
  conversar(ctxA, '573001112244', 'chaparral', [
  ...
  flows.handle('573001112244', '3', ctxB, 'chaparral');
  const rB = flows.handle('573001112244', String(otra), ctxB, 'chaparral');
